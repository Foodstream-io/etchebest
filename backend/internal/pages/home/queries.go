...
import (
	"fmt"
	mappers2 "github.com/Foodstream-io/etchebest/internal/mappers"
	"strings"
	"time"
...
func getFeaturedLive(db *gorm.DB) (*dto.LiveDTO, error) {
	...
	dtoLive := mappers2.LiveToDTO(live)
...
func getLivesByTab(db *gorm.DB, tab string, page, limit int) ([]dto.LiveDTO, int64, error) {
	...
	for _, r := range rows {
		lives = append(lives, mappers2.LiveToDTO(r))
	}
...
func getFeaturedChefs(db *gorm.DB, limit int) ([]ChefHighlight, error) {
	...
	for _, r := range rows {
		chefs = append(chefs, ChefHighlight{
			UserDTO:          mappers2.UserToDTO(r.User),
